...
<div class="innerPane2" style="width: 99%; height: 300px;">
      ...
      <tr>
        <td>
                 <table width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>                 
                    <td width="19%"  class="fieldNames" style="text-align:right"><s:text name='execue.keywords.keyword.heading' /></td>
                    <td width="3%">:</td>                    
                    <s:if test="#mode == 'add'">
					...
					<s:else>
						<td width="68%"><s:textfield name="keyWord.word"
							cssClass="textBox" id="keyWord.word" readonly="true" /></td>
					</s:else>
                  </tr>
               ...
               <div style="margin-left:70px;">
                  ...
                  <tr>
                    ...
                    <s:if test='%{#pwords.UsersParallelWord=="YES" || #pwords.isInvalidParallelWord=="YES" }'>                    
                       <td width="85%"><input  name="uiParallelWords[<s:property value="#ct.index"/>].parallelWord" type=text class="textBox" id="uiParallelWords[<s:property value="#ct.index"/>].parallelWord" value="<s:property value="parallelWord"/>"></td>
                    </s:if>
                    <s:else>                    
                       <td width="85%"><input  name="uiParallelWords[<s:property value="#ct.index"/>].parallelWord" type=text class="textBox" readonly="readonly" id="uiParallelWords[<s:property value="#ct.index"/>].parallelWord" value="<s:property value="parallelWord"/>"></td>
                    </s:else> 
...
<script>
var existingKeywords=0
$("#keyWord").val(keyWord);
$(document).ready(function() {
						   
existingKeywords=$("table#pWordsTable input[type='text']").length;

	if( $("table#pWordsTable input[type='text']").length==0 && existingKeywords==0 ){ 
			$("#submitKeyword").attr("disabled","disabled");
		}
...
if(cnt>8){
   return false;
	...
	$("#submitKeyword").removeAttr("disabled");										  
	$("#imageField").show();	
	rowContent="<tr><td width='20%' align='right'  class='fieldNames'><input name='uiParallelWords["+cnt+"].checkedState' type='checkbox' value='Y' checked id='uiParallelWords["+cnt+"].checkedState'></td><td width='80%' >"
			   +"<input name='uiParallelWords["+cnt+"].parallelWord' type='text' class='textBox' id='uiParallelWords["+cnt+"].parallelWord'></td></tr>";									   
...
$("table#pWordsTable input[type='checkbox']").click(function(){ 
   if( $("table#pWordsTable input[type='checkbox']:checked").length==0 && existingKeywords==0 ){ 
			$("#submitKeyword").attr("disabled","disabled");
		}	else{
			$("#submitKeyword").removeAttr("disabled");
		}
