...
/*if(src=="../images/admin/btn-open.gif"){
  $('#flowDiagramDiv').slideDown(); 
  $obj.find('img').attr('src','../images/admin/btn-close.gif'); 
  ...
  if(appSize>0){
	...
	*/
	$('#grid').jqGrid({
      ...
      viewrecords: true,
	  gridview: true,
	  colNames:[ 'Select App','ModelId','<s:text name="execue.app.name"/>','<s:text name="execue.app.description"/>','Meta Data','<s:text name="execue.publish.mode"/>','<s:text name="execue.global.status"/>'],
	  colModel:[
...
function showMetadataLink(cellvalue, options, rowObject){
	...
	var $editIcon=$('<img height="17" border="0" style="margin-left:15px;" title="Edit" alt="Edit" src="../images/admin/editIcon.gif">');
	$a.append($editIcon);
	if(cellvalue){ 
		$innerDiv.append($a);
	}else{
		$editIcon.attr("src","../images/admin/editIconfaded.gif");
...
function showShortDesc(cellvalue, options, rowObject){
	...
	$longDescContentCloseButton.append("<a style='clocr:#fff;font-weight:bold;' id='"+lDescClose+rowObject.id+"' href=javascript:closeDesc('"+rowObject.id+"');><img border='0' src='../images/admin/closeButtonForMessage.png' /></a>");
