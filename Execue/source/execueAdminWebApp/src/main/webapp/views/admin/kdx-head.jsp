...
function getConcept(conceptID) {
  ...
  $("#loadingShowInstanceLink").show();
    $.post("swi/showInstances.action?concept.id="+conceptID+"&mode=add", function(data) {
      ...
      $("#dynamicPane").append(data);
      $("#dynamicPane").show();*/
	 showDetails("swi/getAllInstances.action?concept.id="+conceptID+"&mode=add","dynamicPane","post","","showInstance","loadingShowInstanceLink");
...
function getPaneSearchDetails(searchType,searchString){
  ...
  function showSelectedTypeBehaviors(advanced){
	 ...
	 $.each($("input[name='"+behaviours+"']"),function(k,v){ 
				var selectedBehaviour=$(this).attr('value');	 
				var exists=false; 
...
function addRow(advanced){
  ...
  }*/ // Base Relations should be able to be reused by modeller - RG- Sep 23 2011
...
function createAssociation(realizationConceptName,realizationConceptBedId,advanced,rowVals){
			...
			$("#relationDisplayDiv"+advanced).prepend($rowOuterDiv).prepend("<img src='../images/admin/space.jpg' width='500' height='2'  />");
...
function deleteConceptHeirarchy(){     
	// alert('delete'+ $("#selectedConceptId").val());
	var selectedConceptId=$("#selectedConceptId").val();	
	var answer = confirm("Are you sure?")
    if (answer){ 
        $("#deleteButton").hide();
		$("#deleteButtonLoader").show();
		var url ="swi/deleteConceptHeirarchy.action?selectedConceptId="+selectedConceptId;
...
function deleteInstanceHeirarchy(parentConceptId,instanceId){ 
	var answer = confirm("Are you sure?")
    if (answer){ 
        $("#showDeleteLink"+instanceId).hide();
		$("#loadingShowDeleteLink"+instanceId).show();
		var url ="deleteInstanceHeirarchy.action?concept.id="+parentConceptId+"&instance.id="+instanceId;
		 $.post(url,function(data){
              if(data.status){
                getInstancesByPage(parentConceptId)
                $("#showDeleteLink"+instanceId).show();
		        $("#loadingShowDeleteLink"+instanceId).hide();
		       // alert(data.message);
		        $("#actionMessage").empty().append(data.message);
			    setTimeout(function(){$("#actionMessage").hide();}, 3000);
...
function deleteInstancesHeirarchyForConcept(){ 
	var answer = confirm("Are you sure?")
    if (answer){ 
        var selectedConceptId=$("#selectedConceptId").val();	
        $("#deleteInstanceButton").hide();
		$("#deleteInstanceButtonLoader").show();
		var url ="deleteInstancesHeirarchyForConcept.action?concept.id="+selectedConceptId;
		 $.post(url,function(data){
              if(data.status){               
                $("#deleteInstanceButton").hide();
		        $("#deleteInstanceButtonLoader").hide();
		        //alert(data.message);
