...
cd $BATCH_HOME
if [ -e $BATCH_HOME/batchpid-runtime-tables-cleanup ]; then
        Process_PID=`cat $BATCH_HOME/batchpid-runtime-tables-cleanup`;
        tnow=`date +%s`
        tfile=`date +%s -r $BATCH_HOME/batchpid-runtime-tables-cleanup`
        age=$((($tnow-$tfile)/60))
        echo "Age "$age;
        echo $Process_PID;
        if [ $age -ge 120 ] ; then
                echo "delete process";
                rm $BATCH_HOME/batchpid-runtime-tables-cleanup;
                kill -9 $Process_PID;
        else
...
echo "$_RUNJAVA -cp $BATCH_CLASSPATH com.execue.offline.batch.semantification.GenericArticleRuntimeTablesCleanupOfflineBatch 110 111 114 115 &"
$_RUNJAVA -cp $BATCH_CLASSPATH -Xms1024m -Xmx2048m com.execue.offline.batch.semantification.GenericArticleRuntimeTablesCleanupOfflineBatch 110 111 114 115 >> $LOG_HOME/log-runtime-tables-cleanup.txt & 
echo $! > batchpid-runtime-tables-cleanup
