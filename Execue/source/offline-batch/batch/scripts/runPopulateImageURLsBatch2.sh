...
cd $BATCH_HOME
if [ -e $BATCH_HOME/batchpid-image-url-processing-model-2 ]; then
        Process_PID=`cat $BATCH_HOME/batchpid-image-url-processing-model-2`;
        tnow=`date +%s`
        tfile=`date +%s -r $BATCH_HOME/batchpid-image-url-processing-model-2`
        age=$((($tnow-$tfile)/60))
        echo "Age "$age;
        echo $Process_PID;
        if [ $age -ge 120 ] ; then
                echo "delete process";
                rm $BATCH_HOME/batchpid-image-url-processing-model-2;
                kill -9 $Process_PID;
        else
...
echo "$_RUNJAVA -cp $BATCH_CLASSPATH com.execue.offline.batch.imageprocessing.PopulateImageURLsOfflineBatch 111 2 &"
$_RUNJAVA -cp $BATCH_CLASSPATH -Xms1024m -Xmx2048m com.execue.offline.batch.imageprocessing.PopulateImageURLsOfflineBatch 111 2 >> $LOG_HOME/log-image-url-processing-model-2.txt & 
echo $! > batchpid-image-url-processing-model-2
