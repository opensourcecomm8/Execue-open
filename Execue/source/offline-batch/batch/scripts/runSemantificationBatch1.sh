...
cd $BATCH_HOME
if [ -e $BATCH_HOME/batchpid-content-semantification-model-1 ]; then
        Process_PID=`cat $BATCH_HOME/batchpid-content-semantification-model-1`;
        tnow=`date +%s`
        tfile=`date +%s -r $BATCH_HOME/batchpid-content-semantification-model-1`
        age=$((($tnow-$tfile)/60))
        echo "Age "$age;
        echo $Process_PID;
        if [ $age -ge 120 ] ; then
                echo "delete process";
                rm $BATCH_HOME/batchpid-content-semantification-model-1;
                kill -9 $Process_PID;
        else
...
echo "$_RUNJAVA -cp $BATCH_CLASSPATH com.execue.offline.batch.semantification.GenericArticleSemantificationOfflineBatch 110 1 &"
$_RUNJAVA -cp $BATCH_CLASSPATH -Xms1024m -Xmx2048m com.execue.offline.batch.semantification.GenericArticleSemantificationOfflineBatch 110 1 >> $LOG_HOME/log-content-semantification-model-1.txt & 
echo $! > batchpid-content-semantification-model-1
