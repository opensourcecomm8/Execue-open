...
$(document).ready(function () {
	...
	$("#middleTableReports").css("marginLeft",posLeftMiddleTable+"px");
...
function chartView(chartName){
   ...
   $("#scatterChartIconView").hide();
   $("#"+chartName).attr('class','active');
   if(chartName == 'DetailGrid' && $("#"+chartName).html() == ""){
   	$("#"+chartName).html('<div id="pleaseWaitDiv" style="margin: 100px 200px; width: 110px;"><img id="waiting_img" src="images/main/Loader-main-page-3.gif" width="32" height="32" /></div>');
...
function showHierarchyGrid(chartName,hierarchySummaryId){
   $("#scatterChartIconView").hide();
   $("#disclaimerDetails").hide();
   $(".active").attr('class','inactive');   
   $("#"+chartName).attr('class','active');
   if($("#"+chartName).html() == ""){
   //$("#"+chartName).html('<div id="pleaseWaitDiv" style="margin: 100px 200px; width: 110px;"><img id="waiting_img" src="images/main/Loader-main-page-3.gif" width="32" height="32" /></div>');
   	$.get("showHierarchyReportGrid.action?hierarchySummaryId="+hierarchySummaryId, function (data) {
   	    $("#"+chartName).empty().html(data);
   	 });
   }   
}
function showScatterChart(chartName,aggregatedQueryId){	
    $("#disclaimerDetails").hide(); 
    $('#charts div').attr('class','inactive'); 
    $("#fx_"+chartName).attr('class','active'); 
    if($("#fx_"+chartName).is(":empty")){
       	$("#fx_"+chartName).html('<div id="pleaseWaitDiv" style="margin: 100px 200px; width: 110px;"><img id="waiting_img" src="images/main/Loader-main-page-3.gif" width="32" height="32" /></div>');
	   	$.get("showScatterChart.action?agQueryIdList="+aggregatedQueryId,function(data){
	   	       $("#fx_"+chartName).empty().html(data);
	   	  });
   	  }
   }

function showScatterChartSubIcons(agQueryIds){
   $("#disclaimerDetails").hide();
   $(".active").attr('class','inactive'); 
   if(agQueryIds.length==1){
     activateDefaultScatterChart(agQueryIds);
   }else{
       var defaultDivToBeAcivated="";
	   $("#scatterChartIconView").show();
	   $("#scatterChartIconView").attr('class','active');  
	   $("#scatterChartIconView").html('<div id="pleaseWaitDiv" style="margin: 100px 200px; width: 110px;"><img id="waiting_img" src="images/main/Loader-main-page-3.gif" width="32" height="32" /></div>');
	   $.get("showScatterChartSubIcons.action?agQueryIdList="+agQueryIds, function (data) {     	
	     	$("#scatterChartIconView").html(data);  
	         activateDefaultScatterChart(agQueryIds.toString().split(',')[0]);   	
	   	}); 
   	}  
}
function activateDefaultScatterChart(aggregatedQueryId){ 
     var chartName= "ScatterChart"+aggregatedQueryId;
      showScatterChart(chartName,aggregatedQueryId);  
}
...
var assetDetailId=0;
$(document).ready(function(){
if($("#IconView img").length<2){ $("#showAllA").css("display","none");}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" id="disclaimerTable">
  ...
  <tr>
    ...
    <td align="right" valign="bottom" style="padding-right:5px;"><a href="#"  id="disclaimerClose" ><img src="images/main/closeUserInfo.png" border="0"  /></a></td>
  </tr>
</table>
...
<table width="100%" cellpadding="0" cellspacing="0" border="0"
			...
			<tr>
				...
				<table width="100%" border="0" cellspacing="0" cellpadding="3"
					...
					<tr>
						...
						<div id="showWelcome"
							style="padding-right: 20px; color: #333; padding-top: 0px;font-size:14px; font-family: 'Trebuchet MS', Arial, Helvetica, sans-serif; text-align: right; width: 200px; float: right;">
						</div>
						</td>
						...
						<a href="<c:url value='/j_spring_security_logout'/>"
							...
							style="display: none;"><img src="images/main/loaderTrans.gif"></span>
						</td>
						<td width="1%"></td>
					</tr>
				</table>
	...
	<tr>
		<td class="technology-1"  height="60">
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">
	<tr>
		<td height="25" colspan="2" align="center"
			background="images/main/bc-bg.jpg" valign="top">
			...
			<tr>			
				<td height="25" align="left" valign="top"><script
					type="text/javascript">
					    ...
					    if(history.length > 1){
					       var assetSise='<s:property value="%{#parameters.assetSize}"/>';
					       if(assetSise==1){
					         window.document.write('<span style="padding-left:0px;color:#333;font-size:14px;"> <a href="#" class="breadCrumbLink" id="backTohome" onclick="backToHomePage();">Home</a> <Span class="textArrow"> >> </Span><span class="breadCrumbNoLink">Reports</span></span></td><td align="right" valign="top" >&nbsp;');
					       }else{
 ...
 <tr>
 <td valign="top" style="padding-top:5px;height:1000px;">  
<div style="width: 94%; margin-left: 20px;margin-right:30px;padding-left:30px; background-color: #FFF;" >
<table width="98%" style="margin-top: 0px" border="0" cellspacing="0" cellpadding="0" align="left" height="100%" id="middleTableReports">
  <tr><td>
  	<div id="errorMessage" style="color: #FF0000"><s:actionerror /><s:fielderror /></div>
  </td>
  </tr>
  <tr>
    <td id="reportsLeftPanel"  valign="top" style="padding-top:0px;" >
...
<table style="margin-top:0px;" width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    ...
    <div style="padding-bottom: 4px;float:right;">
...
<table width="auto" style="margin-top:0px;" border="0" cellspacing="0" cellpadding="0"
	align="left">
	<tr>
		<td valign="top" >
		<table align="center" style="margin-top:0px;">
			<tr>
				...
				<div id="IconView"><s:if test="reportListWrapper != null">
					...
					<s:iterator value="reportListWrapper.htmlReports" id="thumbnails">
						<s:if test="#thumbnails.id != 'DetailGrid' && !#thumbnails.id.contains('hierarchyReportGrid') && #thumbnails.thumbnail != null ">
					  ...
					  <s:iterator value="reportListWrapper.chartFxReports"
						id="chartThumbnails">
						<s:if
								test='%{#chartThumbnails.id.contains("ScatterChart")==false}'>
								<div
					...
					<s:iterator value="reportListWrapper.htmlReports" id="thumbnails">										
					   <s:if test='%{#thumbnails.id.contains("hierarchyReportGrid")}'>						  
							<div
			...
			<tr>
				...
				<table width="auto" border="0" cellspacing="0" cellpadding="0">
					<tr>
						<td align="center" width="60" valign="top" height="30" style="padding-top: 6px;" ><font face="arial">
                        <a id="showAllA" onclick="showAll(this);" style="margin-bottom: 22px; cursor: pointer; text-decoration: underline;">Show all</a></font></td>
...
<jsp:include page="../../../../views/main/footer-search.jsp" />
