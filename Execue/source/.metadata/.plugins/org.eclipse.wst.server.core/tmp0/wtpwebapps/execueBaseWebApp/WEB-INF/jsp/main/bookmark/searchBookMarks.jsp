...
<link rel="stylesheet" href="css/main/jquery.treeview.css" type="text/css"
	media="screen" />
<style>
...
<script type="text/javascript" src="<c:out value="${basePath}"/>/js/main/jquery.treeview.js"></script>
<script type="text/javascript" src="<c:out value="${basePath}"/>/js/main/jquery.cookie.js"></script>
<script type="text/javascript" src="<c:out value="${basePath}"/>/js/main/bookmark.js"></script>
<%
...
<table width="320" border="0" cellspacing="0" cellpadding="3">
	...
	<tr>
		...
		<div class=searchEnd id=searchIcon><a href="#"><img
			src="<c:out value="${basePath}"/>/images/main/searchEndIndex.gif" name="Image2" border="0" id="Image2"
	...
	<tr>
		...
		<img src="<c:out value="${basePath}"/>/images/main/showFoldersButton.jpg" value="Show Folders"
			style="cursor: pointer;" /> <img src="<c:out value="${basePath}"/>/images/main/cancelButton.jpg"
		...
		<table border="0" cellpadding="0" cellspacing="0" align="left">
					...
					<s:iterator value="folderList">
						...
						<div >
							...
							style="text-align: left;width:20px;white-space:nowrap;"><s:property value="name" /></span>
                             ...
                             <span> <a title="Delete Folder" alt="Delete Folder" style="cursor:pointer;"   onclick="return deleteSavedFolder('folder_<s:property value="id"/>');" href="#"><img src="<c:out value="${basePath}"/>/images/main/deleteFolder.gif" border="0" /></a> </span> 
						...
						<ul>
							<s:iterator value="uiBookmarks" id="uiBookmarkIds">
                                ...
                                <div >
                                    ...
                                    <span> <a title="Delete Query" alt="Delete Query" style="cursor:pointer;"   onclick="return deleteSavedQuery('<s:property value="id"/>');" href="#"><img src="<c:out value="${basePath}"/>/images/main/deleteButt1.gif" border="0" /></a> </span>
			...
			<tr id="closeSearchBookmarks">
				...
				<table width="10%" border="0" cellspacing="0" cellpadding="5"
					align="right">
					<tr>
						<td><img src="<c:out value="${basePath}"/>/images/main/closeNewButton.jpg" name="closeButtonId"
...
<script>
$(function(){		   
		var appId=$("#selectApp option:selected").attr("value");
		if(appId){
			$.each($("ul#browser li span.file"),function(k,v){
						var aid=$(this).attr("appid");												   
						if(aid!=appId){
							$(this).find("span.dVal").css("color","#DFDFDF");
							$(this).find("span.dVal").removeAttr("onclick");
							$(this).find("span.dVal").attr('title', 'Not applicable for this page');
							$(this).find("span.dVal").attr('alt', 'Not applicable for this page');
						}										   
	  ...
	  if(type=="string"){
	    $("#bookMarkSearchResult").append(jsonData);
		    ...
		    $.each(jsonData, function(i,item) {	
		         ...
		         if(appId){
		          if(appId != item.applicationId){
			           bookmarkVal="<div><span>"+count+".</span><a id='"+item.id+"' href='#' bookmarkValue='"+item.value+"'  title='Query does not belong to selected application' alt='Query does not belong to selected application'>"+x+"</a></div>";
		          }	
				 }				
