	...
	$.fn.showExecueReport = function(o) {
		...
		function receiveStatusData(data) {
			if (data.reportStatus == 'PROCESSING') {
				...
				options.hierarchySummaryIds= data.hierarchySummaryIds;
				// options.relevance = data.relevance;
				options.error = data.error;
				options.type = data.aggregateQueryType;
		...
		function displayQuery() {
			...
			if (sqlQuery != null) {
				$("#a_" + options.aid+ "_" + options.bqId).show();
			}
		}
		...
		function displayProcessing() {
			...
			if(options.assetSize==1){
			   processingId = '#' + baseid + "_processing";
			  $(processingId).html('Processing...' + $img);
			}else{				
		...
		function display() {
			...
			if(options.assetSize==1){
				var baseid = $me.attr('id');				
				$('#' + baseid + "_pseudoStatement").empty();
				$('#' + baseid + "_query").empty();
				var id = '#' + baseid + "_header a";
				var link=$(id).attr("href");				
				link=link+"&assetSize="+options.assetSize;
		...
		function displayHeader() {
				...
				$.each(reportList, function(i, reportGroupList) {
					...
					if (reportGroupList.reportTypes == 98) {
						...
						imageResult = imageResult
								...
								+ options.imgPath + reportGroupList.imageUrl
								+ "></a>";
					}else if (reportGroupList.reportTypes[0] == 60) {
						
					$.each(options.hierarchySummaryIds, function(i,hierarchyId){
						var actionURL = '<a class="reportLink" href="javascript:showReports(\''
								+ reportGroupList.linkUrl + '?agQueryIdList=';
						imageResult = imageResult + actionURL + aggregateIds;
						imageResult = imageResult						
			...
			} else if (options.type == 'NO_SUMMARY') {
				$(id).append(options.reportHeader);
				$("#reportId_" + options.aid + "_" + options.bqId)
						.val(options.reportHeader);
			} else {
