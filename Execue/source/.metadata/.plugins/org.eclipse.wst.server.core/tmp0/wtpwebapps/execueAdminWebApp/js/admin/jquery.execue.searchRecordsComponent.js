...
var $me = undefined;
var $ImageSearch=$('<img src=../images/admin/searchEnd.gif name=ImageSearch2 border=0 />');
...
if($opts.topSize > 0){
   ...
   $me = $(this);
	...
	function getSearchResults(actionName,method,async){  // used to send action name , div name and get the details 
		...
		if(validate){ 
			$("#"+$targetDivName).css("background","url(../images/admin/loaderTrans.gif) no-repeat center center");
			$.ajax({url:actionName,
	...
	function displayData(data){
		...
		$("#"+$targetDivName).css("background","none");
		$ul.hide();
		//alert(data);
	}
