...
function processColumnMembersJsonData(data) {
	if (data != null) {
		...
		$("<span></span>")
				.html('<img src="../images/admin/bullet_toggle_minus.png" width="16" height="16" align="middle"  />')
				.appendTo($data);
...
function createMappingDataElement(mapping) {
	...
	$data.append("<span class='mapArrow'></span>")
			.append("<img src='../images/admin/arrowRight.jpg' align='middle' />");
	...
	$data.append("<img src='../images/admin/1pix.jpg' width='5' height='5' >");
...
function instanceClick() {
	...
	if ($instanceInput.val() != $inputVal) {
		...
		$instanceInput.focus();
		$me
				.append("<span class='noUL'><img src='../images/admin/1pix.jpg' width='5' height='5' ></span>");
		...
		if ($editFlag) {
			...
			$newInstanceLink = $("<span class='addInstanceLink'>New Instance</span>")
					.click(function() {
						$(this).html('<img src=../images/admin/loaderTrans.gif  />');
			...
			$(this.parentNode)
					.append("<img src='../images/admin/1pix.jpg' width='5' height='5' border='0' >");
			$newInstanceLinkLoader = $("<span id=loadingAddInstances"
					+ instancedata.bedId
					+ "Link style=display:none;><img src=../images/admin/loaderTrans.gif  /></span>");
...
function showMapsSuggested() {
	...
	arrowImage = "<img src=images/admin/arrowRight.jpg alt='Edit Concept' width=25 align=middle  height=25 border=0 />"
	...
	$('li', tree.get(0)).each(function() {
		...
		if (subbranch.size() > 0) {
			if (subbranch.eq(0).css('display') == 'none') {
				...
				$(this)
						.prepend('<img src="../images/admin/bullet_toggle_plus.png" width="16" height="16" class="expandImage" />');
				// }
			} else {
				// if($(this).children(".expandImage")[0]==undefined){
				$(this)
						.prepend('<img src="../images/admin/bullet_toggle_minus.png" width="16" height="16" class="expandImage" />');
				// }
			}
		} else {
			$(this)
					.prepend('<img src="../images/admin/spacer.gif" width="1" height="1" class="expandImage" />');
		}
	});
	$('img.expandImage', tree.get(0)).click(function() {
		if (this.src.indexOf('spacer') == -1) {
			...
			if (subbranch.css('display') == 'none') {
				subbranch.show();
				this.src = '../images/admin/bullet_toggle_minus.png';
			} else {
				subbranch.hide();
				this.src = '../images/admin/bullet_toggle_plus.png';
			}
		}
	});
}
				...
				if (subbranches.size() > 0) {
					...
					if (subbranch.css('display') == 'none') {
						...
						this.expanderTime = window.setTimeout(function() {
							...
							$('img.expandImage', targetBranch.parentNode).eq(0)
									.attr('src',
											'images/admin/bullet_toggle_minus.png');
							$.recallDroppables();
						}, 500);
					}
				}
...
function checkforMapsToSave(){ 
var status=false;
	if($("#mapsContent div.sugClassName").length>0){
		var  st=confirm("Do you want save the changes ?");
		if(st){
		status=true;
		$("#confirmMappingsId").trigger("click");
		return status;	
		}
	}
	return status;	
