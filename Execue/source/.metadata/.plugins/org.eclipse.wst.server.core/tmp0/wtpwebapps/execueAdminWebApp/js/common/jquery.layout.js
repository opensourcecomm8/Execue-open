 ...
 */
	...
	,	defaults: { // default options for 'all panes' - will be overridden by 'per-pane settings'
		...
		,	spacing_open:			10			// space between pane and adjacent panes - when pane is 'open'
		,	spacing_closed:			10			// ditto - when pane is 'closed'
	...
	var initHandles = function () {
		...
		$.each(c.borderPanes.split(","), function() {
			...
			if (o.closable) {
				...
				;
//alert(tClass +" "+ tClass+_pane +" "+ tClass+_state +" "+ tClass+_pane+_state );
 ...
 */
	...
	var toggle = function (pane) {
		...
		if (s.isHidden)
			...
			if (typeof collistWidth != "undefined"){

			collistWidth=collistWidth-200;
