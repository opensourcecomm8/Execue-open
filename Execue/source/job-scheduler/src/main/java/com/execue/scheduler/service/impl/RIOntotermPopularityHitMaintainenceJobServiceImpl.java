...
/**
 * This service schedules the updation of the popularity hit of the RIOntoTerms
 * 
 * @author Vishay
 * @version 1.0
 * @since 10/09/09
 */
public class RIOntotermPopularityHitMaintainenceJobServiceImpl extends BaseJobService implements
         IRIOntotermPopularityHitMaintainenceJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleRIOntoTermPopularityHitMaintainenceJob (
            PopularityHitMaintenanceContext popularityHitMaintenanceContext) throws KDXException {
      JobRequest jobRequest = null;
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.RI_ONTO_TERM_POPULARITY_HIT_UPDATION);
         XStream xStream = new XStream();
         String riOntoTermPopularityInputXML = xStream.toXML(popularityHitMaintenanceContext);
         ...
         jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.RI_ONTO_TERM_POPULARITY_HIT_UPDATION,
                  riOntoTermPopularityInputXML, JobStatus.PENDING, new Date(), popularityHitMaintenanceContext
                           .getUserId());
         ...
         throw new KDXException(
                  SWIExceptionCodes.UPDATE_RI_ONTO_TERM_POPULARITY_HIT_UPDATION_JOB_SCHEDULING_FAILURE_CODE, e);
