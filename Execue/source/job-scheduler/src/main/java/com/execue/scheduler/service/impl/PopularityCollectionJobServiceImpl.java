...
public class PopularityCollectionJobServiceImpl extends BaseJobService implements IPopularityCollectionJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long schedulePopularityCollectionJob (PopularityRestorationContext popularityRestorationContext)
            throws SWIException {
      JobRequest jobRequest = null;
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.POPULARITY_COLLECTION);
         XStream xStream = new XStream();
         String popularityCollectionInputXML = xStream.toXML(popularityRestorationContext);
         ...
         jobRequest = ExecueBeanManagementUtil
                  .prepareJobRequest(JobType.POPULARITY_COLLECTION, popularityCollectionInputXML, JobStatus.PENDING,
                           new Date(), popularityRestorationContext.getUserId());
         ...
         throw new KDXException(SWIExceptionCodes.POPULARITY_COLLECTION_JOB_SCHEDULING_FAILURE_CODE, e);
