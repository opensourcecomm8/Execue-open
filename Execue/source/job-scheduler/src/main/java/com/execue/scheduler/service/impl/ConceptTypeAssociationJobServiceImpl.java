...
public class ConceptTypeAssociationJobServiceImpl extends BaseJobService implements IConceptTypeAssociationJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleConceptTypeAssociationJob (ConceptTypeAssociationContext conceptTypeAssociationContext)
            throws KDXException {
      JobRequest jobRequest = null;
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.CONCEPT_TYPE_ASSOCIATION);
         XStream xStream = new XStream();
         String concetTypeAssociationInputXML = xStream.toXML(conceptTypeAssociationContext);
         ...
         jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.CONCEPT_TYPE_ASSOCIATION,
                  concetTypeAssociationInputXML, JobStatus.PENDING, new Date(), conceptTypeAssociationContext
                           .getUserId());
         ...
         throw new KDXException(SWIExceptionCodes.CTA_MAINTENANCE_JOB_FAILED, execueJobSchedulerException);
      } catch (QueryDataException queryDataException) {
         throw new KDXException(SWIExceptionCodes.CTA_MAINTENANCE_JOB_FAILED, queryDataException);
      }
