 ...
 * @author Vishay
 */
public class CubeUpdationJob extends BaseLineJob {

   private static final Logger                   logger = Logger.getLogger(CubeUpdationJob.class);
   ...
   public void process (ExecueJobData jobData) throws ExecueJobSchedulerException {
      logger.debug("Invoking Cube Update Job [" + jobData.getJobName() + "]");
      ...
      try {
         ...
         assetOperationDetail = AnswersCatalogPlatformServiceHelper.prepareAssetOperationDetail(
                  ...
                  jobData.getJobRequest().getId(), AnswersCatalogOperationType.CUBE_UPDATION,
         ...
         getAnswersCatalogManagementQueueService().updateOperationStatus(answersCatalogManagementQueueId,
                  ACManagementOperationStatusType.INPROGRESS);
         CubeUpdationOutputInfo cubeUpdationOutputInfo = getAnswersCatalogUpdationService().cubeupdate(
                  answersCatalogUpdationContext);
         if (cubeUpdationOutputInfo.isUpdationSuccessful()) {
            ...
            if (logger.isDebugEnabled()) {
               logger.debug("Failed to update Cube");
            }
