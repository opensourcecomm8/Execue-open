...
/**
 * This service schedules the
 * 
 * @author Vishay
 * @version 1.0
 * @since 22/08/10
 */
public class SDXSynchronizationJobServiceImpl extends BaseJobService implements ISDXSynchronizationJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleSDXSynchronizationJob (SDXSynchronizationContext sdxSynchronizationContext)
            throws SDXException {
      JobRequest jobRequest = null;
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.SDX_SYNCHRONIZATION);
         XStream xStream = new XStream();
         String sdxSynchronizationInputXML = xStream.toXML(sdxSynchronizationContext);
         ...
         jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.SDX_SYNCHRONIZATION,
                  sdxSynchronizationInputXML, JobStatus.PENDING, new Date(), sdxSynchronizationContext
                           .getUserId());
         ...
         throw new SDXException(SWIExceptionCodes.MEMBER_SYNCHRONIZATION_JOB_SCHEDULING_FAILURE_CODE, e);
