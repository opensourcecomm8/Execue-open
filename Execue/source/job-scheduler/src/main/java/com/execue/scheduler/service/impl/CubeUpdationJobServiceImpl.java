...
/**
 * This service schedules the cube updation job
 * 
 * @author Vishay
 * @version 1.0
 * @since 25/06/09
 */
public class CubeUpdationJobServiceImpl extends BaseJobService implements ICubeUpdationJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleCubeUpdationJob (AnswersCatalogUpdationContext answersCatalogUpdationContext)
            throws AnswersCatalogException {
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.CUBE_UPDATION);
         XStream xStream = new XStream();
         String cubeUpdationXML = xStream.toXML(answersCatalogUpdationContext);
         ...
         JobRequest jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.CUBE_UPDATION, cubeUpdationXML,
