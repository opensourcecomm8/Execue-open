 ...
 * @since 10/09/09
 */
public class UpdateSFLWeightBasedOnSecondaryWordJobServiceImpl extends BaseJobService implements
         IUpdateSFLWeightBasedOnSecondaryWordJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleUpdateSFLTermTokenWeightBasedOnSecondaryWordJob (
            SFLTermTokenWeightContext sflTermTokenWeightContext) throws KDXException {
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.SFL_WEIGHT_UPDATION_BY_SECWORD);
         ...
         JobRequest jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.SFL_WEIGHT_UPDATION_BY_SECWORD,
         ...
         throw new KDXException(SWIExceptionCodes.UPDATE_SFL_WEIGHT_BY_SECWORD_JOB_SCHEDULING_FAILURE_CODE, e);
