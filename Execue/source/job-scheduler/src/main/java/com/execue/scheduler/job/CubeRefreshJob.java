 ...
 * @author Vishay
 */
public class CubeRefreshJob extends BaseLineJob {

   private static final Logger                   logger = Logger.getLogger(CubeRefreshJob.class);
   ...
   public void process (ExecueJobData jobData) throws ExecueJobSchedulerException {
      logger.debug("Invoking Cube Refresh Job [" + jobData.getJobName() + "]");
      ...
      try {
         ...
         assetOperationDetail = AnswersCatalogPlatformServiceHelper.prepareAssetOperationDetail(
                  ...
                  jobData.getJobRequest().getId(), AnswersCatalogOperationType.CUBE_REFRESH,
         ...
         getAnswersCatalogManagementQueueService().updateOperationStatus(answersCatalogManagementQueueId,
                  ACManagementOperationStatusType.INPROGRESS);
         CubeRefreshOutputInfo cubeRefreshOutputInfo = getAnswersCatalogUpdationService().cubeRefresh(
                  answersCatalogUpdationContext);
         if (cubeRefreshOutputInfo != null) {
            if (cubeRefreshOutputInfo.isRefreshSuccessful()) {
               Asset cubeAsset = cubeRefreshOutputInfo.getCubeCreationOutputInfo().getCubeCreationInputInfo()
                        .getCubeCreationPopulatedContext().getTargetAsset();
               if (logger.isDebugEnabled()) {
                  logger.debug("New Cube Asset Id " + cubeAsset.getId());
               }
            ...
            if (logger.isDebugEnabled()) {
               logger.debug("Failed to refresh Cube");
            }
         }
      } catch (Exception ex) {
         jobData.getJobRequest().setJobStatus(JobStatus.FAILURE);
