 ...
 * @since 25/06/09
 */
public class CubeCreationJobServiceImpl extends BaseJobService implements ICubeCreationJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleCubeCreationJob (CubeCreationContext cubeCreationDetails) throws AnswersCatalogException {
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.CUBE_CREATION);
         XStream xStream = new XStream();
         String cubeCreationXML = xStream.toXML(cubeCreationDetails);
         ...
         long userId = cubeCreationDetails.getUserId();
         JobRequest jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.CUBE_CREATION, cubeCreationXML,
