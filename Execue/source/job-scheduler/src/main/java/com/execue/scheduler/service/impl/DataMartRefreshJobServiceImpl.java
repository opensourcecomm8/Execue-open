...
/**
 * This service schedules the mart refresh job
 * 
 * @author Vishay
 * @version 1.0
 * @since 25/06/09
 */
public class DataMartRefreshJobServiceImpl extends BaseJobService implements IDataMartRefreshJobService {

   private IExecueJobSchedulerService execueJobSchedulerService;
   private IJobDataService            jobDataService;

   public Long scheduleMartRefreshJob (AnswersCatalogUpdationContext answersCatalogUpdationContext)
            throws AnswersCatalogException {
      try {
         ExecueJobData jobData = new ExecueJobData();
         populateJobData(jobData, JobType.MART_REFRESH);
         XStream xStream = new XStream();
         String martRefreshXML = xStream.toXML(answersCatalogUpdationContext);
         ...
         JobRequest jobRequest = ExecueBeanManagementUtil.prepareJobRequest(JobType.MART_REFRESH, martRefreshXML,
