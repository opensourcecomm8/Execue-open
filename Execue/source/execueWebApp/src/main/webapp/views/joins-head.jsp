    ...
    //  if(leftTable==rightTable){alert("this table already selected");return false;}
      ...
      $.getJSON("swi/showTableColumnsForJoin.action?assetId="+jsAssetId+"&tableName="+leftTable,function (jsonData) {
        ...
        $.each(jsonData, function(item) {
          ...
          colContent="<LI class=leftColumn id="+this.columnName+" ><IMG class=folderImage src=../images/columnIcon.jpg><SPAN class=leftColumnHolder>"+this.columnName+"</SPAN> ";
          //left column list creation
...
function populateRightTable(){ 
   ...
   // if(leftTable==rightTable){alert("this table already selected"); return false;}
    ...
    //alert(rightTable);
     $.getJSON("swi/showTableColumnsForJoin.action?assetId="+jsAssetId+"&tableName="+rightTable,function (jsonData) {
            ...
            $.each(jsonData, function(item) {
          //alert(this.name+"::"+this.id); 
      colContent="<LI class=rightColumn ><IMG class=folderImage src=../images/columnIcon.jpg><SPAN class=rightColumnHolder id="+this.columnName+">"+this.columnName+"</SPAN> ";
 ...
 function createJoin(){
    ...
    for(i=0;i<$joinsArrayArray.length;i++){
        ...
        count=0;
		tempContent="";
        tempContent="<tr><td width='20' border='1'><a class='del' href='#' id='"+i+"'><img src='../images/remove.jpg' border='0' alt='delete' title='delete'/></a></td>";
        for(key in temp){
