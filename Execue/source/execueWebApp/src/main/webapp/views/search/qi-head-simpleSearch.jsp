...
<c:set var="basePath" value="<%=request.getContextPath()%>" />
<link href="<c:out value="${basePath}"/>/css/qiStyle_new.css" rel="stylesheet" type="text/css" />
<link rel="stylesheet" href="<c:out value="${basePath}"/>/css/roundedSearch.css" type="text/css" />
<link rel="stylesheet" href="<c:out value="${basePath}"/>/css/jquery.autocomplete.css" type="text/css" />
<script language="JavaScript" src="<c:out value="${basePath}"/>/js/goog_analytics.js"></script>
<script language="JavaScript" src="<c:out value="${basePath}"/>/js/jquery.js"></script>
<script type="text/javascript" src="<c:out value="${basePath}"/>/js/jquery.ui.all.js"></script>
...
function getAppExamples(){
	...
	if(applicationId != -1){
		jsonExamples = $.get("<c:out value="${basePath}"/>/querySuggest/showAppSpecificExamples.action?applicationId="+applicationId,
		function(data) {
		    var data=eval(data);
...
<script>
   ...
   function showPublisherInfo(admin,publisher) {
			...
			$("#publisherTd").show();
			$("#changePassword").show();
			$("#adminId").attr("href","<c:out value="${basePath}"/>/swi/showSearchAppsDashboard.action");
			}
		}
