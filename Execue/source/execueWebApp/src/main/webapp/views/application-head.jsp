...
width:20px;
float:left;
background:url(../images/deleteButt1.gif) no-repeat;
	cursor:pointer;
}
#snapShot{
width:370px;
...
checkMode();
  if (returnedData.status) {
	 ...
	 $("#applicationQueryExampleTable").show();	 
	  $("#dynamicImageOuterDiv").show().empty().html("<img src='../images/loading.gif'  style='padding-left:0px;' />");
	...
	setTimeout(function(){$("#dynamicImageOuterDiv img").width(90).height(90).css("paddingLeft","5px").attr("src","getImage.action?applicationId="+returnedData.applicationId+"&appImageId="+returnedData.applicationImageId);},2000);
		...
		setTimeout(function(){$("#dynamicImageOuterDiv img").width(90).height(90).css("paddingLeft","5px").attr("src","../images/noImage-icon.gif");},2000);	
  }
...
if($.trim($("#example").val()).length==0){
	$("#alertMessaage").show().html("Please Enter example");
	return false;
}else{
	$("#alertMessaage").hide();
}
