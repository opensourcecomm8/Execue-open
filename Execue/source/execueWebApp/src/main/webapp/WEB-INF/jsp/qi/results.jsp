...
<script type="text/javascript"
    ...
    <div id="pageLoader" style="display:none;padding:4px;padding-top:5px;z-index:1000000;position:absolute;border:none;width:50px;height:50px;"><div style="width:32px;;margin:auto;"><img id="waiting_img2" src="images/Loader-main-page-3.gif" width="32" height="32" /></div><div style="width:100%;text-align:center;">Loading</div></div>
<div style=" margin: auto; margin-bottom: 2px; margin-top: 2px;" class="outerDiv">
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" height="10%" >
	<tr>
		<td height="20" class="normalText" width="50%"><span
			...
			id="backTohome" onclick="backToHomePage();" class="breadCrumbLink"><s:text name='execue.home.link' /></a><Span
			class="textArrow"> >> </Span> <span class="breadCrumbNoLink"><s:text name='execue.global.results.link' /></span>
...
<table border="0" cellpadding="0" cellspacing="0" width="100%" >
	...
	<tr>
		<td background="images/message-bar_r1_c1.png"><img
			name="messagebar_r1_c1" src="images/message-bar_r1_c1.png"
			width="600" height="4" border="0" id="messagebar_r1_c1" alt="" /></td>
	</tr>
	<tr>
		<td background="images/message-bar_r2_c1.png" height="auto">
		...
		<table width="99%" border="0" cellspacing="0" cellpadding="0"
			...
			<tr id="trRequest">
				<td height="22"><a class="requestTitle"><s:text name='execue.global.request.text' /> </a><s:property
					...
					src="images/loaderTrans.gif"></span><span style="color:#3F5356;">]</span>
                    <!-- cachec code -->
	...
	<tr>
		<td background="images/message-bar_r3_c1.png"><img
			name="messagebar_r3_c1" src="images/message-bar_r3_c1.png"
			width="600" height="1" border="0" id="messagebar_r3_c1" alt="" /></td>
	</tr>
</table>
</div>
          ...
          <tr>
			...
			<tr>
				...
				<table width="90%" border="0" cellspacing="0" cellpadding="0">
					...
					<!-- tr>
				<td height="25"><form name="backToHome" method="post"><div id="back" style="padding-left:5px;">
               
                <a href="#" 
			id="backTohome" onclick="backToHomePage();">Home</a> <Span class="textArrow"> >> </Span> Results </div><s:hidden id="requestString" name="requestString"/>
			...
			<tr>
				<td><img src="images/space.jpg" width="10" height="3"></td>
			</tr>
			<tr>
				<td>
            ...
            <!--table border="0" cellspacing="0" cellpadding="0">
					<s:if test="result.tokenCandedates.size !=0">
						<tr>
							<td>
							<form id="dForm" action="processEntities.action" method="post"
								onsubmit="return createRequest();">
							<div id="doYouMean" class="mean"><s:hidden name="request"
								id="requestedUserQueryId" />
							<table width="auto" border="0" cellspacing="0" cellpadding="3">
								<tr>
									<td>Do you mean:</td>
									<s:iterator value="result.tokenCandedates" id="tokenCandidate"
										status="stat">
										<td valign="middle"><STRONG><s:property
											value="%{#tokenCandidate.word}" />: </STRONG></td>
										<td><s:select id="entities[%{#stat.index}].id"
											name="entities.id" list="%{#tokenCandidate.entities}"
											listKey="id" listValue="name"
											onchange="setValues(this,%{#stat.index});">

										</s:select> <s:hidden id="entities[%{#stat.index}].original"
											name="entities[%{#stat.index}].original"
											value="%{#tokenCandidate.word}"></s:hidden> <s:hidden
											id="entities[%{#stat.index}].type"
											name="entities[%{#stat.index}].type" value="x"></s:hidden> <s:hidden
											id="entities[%{#stat.index}].name"
											name="entities[%{#stat.index}].name" value="y"></s:hidden> <script>
						  $(document).ready(function(){
						 tokenCandidateWords[wordCount]='<s:property value="%{#tokenCandidate.word}"/>';
                          globalType[wordCount]=[];
						 
						 $words=[];
						 $hiddenField=$("<input type='hidden'></input>").attr("id","hiddenData"+wordCount).data("data",$words);
						 $("#dForm").append($hiddenField);
						 
						$hF=$("<div ></div>").attr("id","HiddenFieldsToServer"+wordCount)
						$("#dForm").append($hF);
							 });
                         </script> <%
    //int optionCount = 0;
 %> <s:iterator
											value="%{#tokenCandidate.entities}" id="candidateEntity"
											status="entityStat">
											<script>
								 $(document).ready(function(){
								$words[<%//=optionCount%>]=[];
											...
											<%
											   //int wCount = 0;
											%>
											<s:iterator value="%{#candidateEntity.words}"
												status="statWord">

												<script>
								 $(document).ready(function(){
								$words[<%//=optionCount%>][<%//=wCount%>]='<s:property/>';	
								//alert("words:::::"+words[wordCount][<%//=wCount%>]);
								});
                                </script>
												<%
												   //wCount++;
											...
											<script>	 
						    $(document).ready(function(){
                             globalType[wordCount][globalType[wordCount].length]='<s:property value="type"/>';	
							});
                           </script>
					 ...
					 $(document).ready(function(){
                      var name=document.getElementById("entities["+wordCount+"].name");
                      var id=document.getElementById("entities["+wordCount+"].id");
 					  var type=document.getElementById("entities["+wordCount+"].type");
 					  var original=document.getElementById("entities["+wordCount+"].original");
							var opt=id.options.length;
						id.options[opt] = new Option('— Select One —', '');
						id.options[opt].selected=true;

                      name.value=original.value;                   
                      type.value=globalType[wordCount][0];
                      wordCount++;
						});
                      </script>
									</s:iterator>
...
<div ><img src="images/1pix.gif" width="100%" height="5" /></div>
					<div class="showPossibility" ><s:property value="status" />
					<div id="requestRecognisedDiv"
						style="margin-top: 0px; margin-bottom: 0px;">
					<table width="100%" border="0" cellspacing="0" cellpadding="0"
						background="images/line-bg.jpg">
						<c:if test="${!empty assets}">
							<tr id="trRequest">
								<td  width="715" height="20" style="padding-left: 7px;">
                                <s:if test="type=='SI'" >
                                <a class="requestTitle" ><s:text name='execue.resultspage.requestRecognisedBy.text' /></a> 
									...
									style="padding-left: 0px;">&nbsp;<s:text name='execue.resultspage.as.text' />&nbsp;</a><span style="font-size:9pt;"><s:property value="colorCodedRequest" escape="html" /></span>
                                    
                         </s:if>
                          <s:else>
                          Results from <s:property value="appName"/>
                          </s:else>              
                                    
                                    </td>
                                    <td align="left" style="padding-right:20px;">
                                    <s:if test='%{#possibility.applicationType.value == "U"}'>
                                      <a id="viewLink<s:property value='#stPossible.index' />" href="javascript:showHideImageViewU(<s:property value='#stPossible.index' />)">Image View</a></s:if>
                                    </td>
							</tr>
						</c:if>
					</table>
                     ...
                     <s:if test="resultItemType.value==3 && resultItemType.value!=#match" ><div class="resultsmatch"><s:text name="execue.universal.partialMatch" /> ( <s:property value="#possibility.partialMatchCount" /> )</div><s:set name="match" value="resultItemType.value"/> </s:if>  
                      <a  target="_blank" href="<s:property value='headerUrl' />" >
                         ...
                         <s:if test="#status1.last==true">
                                              <div style="width:100%;padding-top:5px;float:left;padding-bottom:5px;"><a href="#" uAppId='<s:property value="appId" />' id="moreResult"><img  border="0"  src="<c:out value="${basePath}"/>/images/app/more-2results.png"></a></div>
                                            </s:if>
                    ...
                    <s:if test="resultItemType.value==2 && resultItemType.value!=#match" ><div class="resultsmatch"><s:text name="execue.universal.unknownMatch" />  ( <s:property value="#possibility.mightMatchCount" /> )  </div><s:set name="match" value="resultItemType.value"/> </s:if>
                     ...
                     <s:if test="resultItemType.value==3 && resultItemType.value!=#match" ><div class="resultsmatch"><s:text name="execue.universal.partialMatch" />  ( <s:property value="#possibility.partialMatchCount" /> ) </div><s:set name="match" value="resultItemType.value"/> </s:if>
                                            ...
                                            <s:if test="#status.last==true" >
                                              <div style="width:100%;padding-top:10px;"><a href="#" uAppId='<s:property value="appId" />' id="moreResultList"><img  border="0"  src="<c:out value="${basePath}"/>/images/app/more-2results.png"></a></div>
                                            </s:if>
						...
						<table border=0 bordercolor=pink width="96%">
							<!-- PINK-->
							<tr valign="middle">
								<td><s:if test='%{reportStatus=="COMPLETED"}'>
									...
									<table border=0 bordercolor=#666 width="90%" cellpadding="0"
										cellspacing="0">
										<tr>
											<td width=15>&nbsp;</td>
										...
										<tr>
										   ...
										   <s:if test="appImageId != -1">
										<img  src="getImage.action?applicationId=<s:property value='appId'/>&appimageId=<s:property value='appImageId'/>" alt="not found" align="bottom" border="0" width="30" height="30" ></img>
										</s:if>   
										<s:else>
										<img  src="images/noImage-icon.gif" alt="not found" align="bottom" border="0" width="30" height="30" ></img>
										</s:else>
											</div>
								...
								query</a> --></td>
										...
										<tr>
											...
											<div id="report_<s:property value="id"/>_query" class="sql"
												style="display: none; padding-left: 15px; overflow: auto; width: 700px; height: 150px">
									...
									<table border=0 bordercolor=#666 width="98%" cellpadding="0"
										...
										<tr>
											...
											<div id="extended_<s:property value="id"/>_note" class="sql"
												style="display: none;  overflow: auto; width: 800px; min-height: 1px;height:auto;">
											</div>
											</td>
										</tr>
										...
										&nbsp;<s:text name='execue.resultspage.appPublishedBy.text' />
                                         ...
                                         <!-- Commented review query --> 
                                            
       <a href="#" title="Review Query" id="a_<s:property value="id"/>_<s:property value='businessQueryId'/>" class="result" style="display:none;"
								onMouseOver="showReviewQry('report_<s:property value="id"/>_<s:property value='businessQueryId'/>_query');"
                                onMouseOut="hideReviewQry('report_<s:property value="id"/>_<s:property value='businessQueryId'/>_query');"
									onClick="stayReviewQry('report_<s:property value="id"/>_<s:property value='businessQueryId'/>_query');">review query</a>
		 ...
		 End Pagination Code --></td>
            ...
            <td width="15%" valign="top" style="padding-top:10px;border-left:1px solid #D3E1F9;">
            <!--               ad content starts -->
                          ...
                          <s:iterator id="rQuery" value="result.relatedUserQueries"  status="st">
                            ...
                            <s:else>
			                	...
			                	<s:if test="#st.last == true">
				                    ...
				                    <img height="15" border="0" width="50" src="<c:out value="${basePath}"/>/images/app/more-2.png"></a>
				                    </div>
				                </s:if>              
                            </s:else>
                       </s:iterator>
				...
				<tr>
					...
					<TR>
						...
						<c:if test="${nCount > 4}">
                                      ...
                                      <div style="float:right;width:100%;padding-top:3px;margin-top:3px;border-top:1px dotted #D3E1F9"><a style="float:right;" href="javascript:showMore('moreNewsDiv','showMoreLink2');" id="showMoreLink2" onfocus="this.blur()"><img height="15" border="0" width="50" src="<c:out value="${basePath}"/>/images/app/more-2.png"></a></div>
...
<div id="userInfoDynamicDivOuter" style="display:none;z-index:10000;width:500px;height:335px;position:absolute;left:150px;top:150px;background-color:#ffffff;padding:5px;" >
<table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
  ...
  <tr>
    ...
    <td align="right" valign="bottom" style="padding-right:5px;"><a href="#"  id="closeUserInfo" ><img src="images/closeUserInfo.png" border="0"  /></a></td>
  </tr>
</table>
...
<div id="showBigImage" 	style="display: none;  overflow: auto; width: auto;; min-height: 1px;height:auto;position:absolute;z-index:10000;left:0px;top:0px;border:3px soild #333;"><table width="100%" border="0" cellspacing="0" cellpadding="0">
  <tr>
    <td bgcolor="#666666" align="right" style="padding-right:3px;"><a href="javascript:closeImage();"><img src="images/closeUserInfo.png" border="0" /></a></td>
  </tr>
  <tr>
    <td width="200" height="200" bgcolor="#FFFFFF" align="center" id="imageTd" style="border:1px solid #D3E1F9;padding:2px;background:#ffffff url(images/craigslist/Loader.gif) center center no-repeat"> <div id="bigImg" style="display:none;" ></div></td>
  </tr>
</table>
...
$(document).ready(function(){
		...
		$("img.listimage").click(function(){
			if($(this).attr("src")!="images/noImage.png"){ 
			...
			$("#showBigImage").css("top",$top+"px");
				...
				$("a.reportLink").click(function(){	
						...
						if($(this).find('img').attr('src')	!="images/CSVFILE.jpg"){					 
...
function showMore(divName, id){
	$("#"+divName).slideDown(); 
	$("#"+id+" img").attr("src","<c:out value="${basePath}"/>/images/app/less-2.png");
	$("#"+id).attr("href","javascript:hideMoreLink('"+divName+"','"+id+"');");
}
function hideMoreLink(divName, id){
	$("#"+divName).slideUp(); 
	$("#"+id+" img").attr("src","<c:out value="${basePath}"/>/images/app/more-2.png");
	$("#"+id).attr("href","javascript:showMore('"+divName+"','"+id+"');");
	
}

function getDynamicCraigsListForm(){
 var craigsListForm = document.createElement("FORM");
 document.body.appendChild(craigsListForm);
 craigsListForm.method = "POST";
 return craigsListForm;
}
$("#moreResult,#moreResultList").click(function(){
   var uAppId=$("#moreResult").attr("uAppId");
   var request=$("#unstructureAppRequestId").val(); 
   var submitForm = getDynamicCraigsListForm();
