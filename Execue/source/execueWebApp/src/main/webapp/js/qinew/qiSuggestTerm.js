		...
		return this.each(function() {
			...
			function add(text, html) {
				if (text.displayName != undefined) {
					 ...
					 * $statName = text["name"]; } } else { stat = false; } }
					 */
			...
			function generateSnapShot() {
				...
				$snapShot.empty();
				// $snapShot.css("float","left");
				// $snapShot.css("cursor","left");
				$snapShot.width(650);
				if (nodata) {
					// alert("c");
					...
					$snapShot
							.append("<span style='float:left;padding-right:9px;'>"
									+ o.snapShotText + "</span>");
					// $snapShot.append('<br>');
				}
				// alert("::"+$snapShot.html().indexOf("</span>"));
				if (nodata) {
					// alert("c");
					$('#tr_' + o.snapShotId).hide();
				}
				$.each($bits, function(index, selectTerm) {
					cnt++;
					$snapBit = $("<div class='snapBit' ></div>");
					$snapBit.bind("mouseover", function() {
						$(this).children(".delLink").css("background-image",
								"url(images/deleteButt1.gif)");
					...
					$link = $('<div class="delLink">&nbsp;</div>')
							// .css("background-image","url(images/deleteButt.gif)")
							.bind('click', function(e) {
								e.stopPropagation();
								dispose(selectTerm['element']);
							});
					// $link.addClass('bit-box');

					if (wordCount == 0 && count > 3) {
						row++;
						if (row >= 2) {
							$snapBit.css("padding-left", "55px");
						}
					}

					if (selectTerm['type'] == "STAT") {
						$snapBit.append("<div style='float:left;'>"
								+ selectTerm['displayName'] + "</div>");
						...
						if (cnt > 0 && cnt < count) {
							$snapShot
									.append("<span style='float:left;padding-right:4px'> ,  </span>");
						}
						// $selTB=$selTB+selectTerm['displayName']+", ";
			...
			this.qiSTGenerateSelectTextXML = function(o) {
				...
				XML.BeginNode();
				XML.Node("modelId", $modelId);
				XML.Node("applicationId", $applicationId);
