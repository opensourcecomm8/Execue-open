...
public class ApplicationAction extends SWIPaginationAction {
   ...
   public String getAllVisibleVerticals () {
      try {
         setVerticals(getVerticalServiceHandler().getAllVisibleVerticals());
      } catch (HandlerException e) {
         e.printStackTrace();
      }
      return SUCCESS;
      ...
      try {
         ...
         if (application.getId() != null) {
            ...
            if (ExecueCoreUtil.isCollectionNotEmpty(models)) {
               ApplicationContext appContext = ExecueBeanManagementUtil.prepareApplicationContext(
                        models.get(0).getId(), application.getId(), application.getName(), null, null, application
                                 .getSourceType());
      ...
      try {
         if (application != null && application.getId() != null) {
            ...
            if (application != null) {
               List<Model> models = applicationServiceHandler.getModelsByApplicationId(application.getId());
               if (ExecueCoreUtil.isCollectionNotEmpty(models)) {
                  ApplicationContext applicationContext = ExecueBeanManagementUtil.prepareApplicationContext(models
                           .get(0).getId(), application.getId(), application.getName(), null, null, application
                           .getSourceType());
