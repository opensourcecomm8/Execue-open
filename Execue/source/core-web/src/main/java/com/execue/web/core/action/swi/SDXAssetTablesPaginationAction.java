...
public class SDXAssetTablesPaginationAction extends SWIPaginationAction {

   private static final Logger logger          = Logger.getLogger(SDXAssetTablesPaginationAction.class);
   ...
   public String processPage () throws ExeCueException {
      try {
         ...
         asset = getSdxServiceHandler().getAsset(getApplicationContext().getAppId(), asset.getName());
         listOfTables = getSdxServiceHandler().getAllAssetTables(asset);
         // set the record count
         getPageDetail().setRecordCount(Long.valueOf(listOfTables.size()));
         assetTables = getProcessedResults(listOfTables, getPageDetail());
         // push the page object into request
         if (logger.isDebugEnabled()) {
            logger.debug("\n" + getPageDetail().toString());
         }
         getHttpRequest().put(PAGINATION, getPageDetail());
         logger.debug("table size is " + assetTables.size());
      ...
      if (ExecueCoreUtil.isCollectionNotEmpty(searchList)) {
         ...
         if (PageSearchType.STARTS_WITH == search.getType()) {
            ...
            for (UITable table : listOfTables) {
               String cDispName = table.getName();
