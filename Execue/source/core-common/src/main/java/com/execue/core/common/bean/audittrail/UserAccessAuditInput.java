/**
 * Licensed to the Execue Software Foundation (ESF) under one
 * or more contributor license agreements.  See the NOTICE file
 * distributed with this work for additional information
 * regarding copyright ownership.  The ESF licenses this file
 * to you under the Execue License, Version 2.0 (the
 * "License"); you may not use this file except in compliance
 * with the License. 
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */


package com.execue.core.common.bean.audittrail;

import java.util.Date;
import java.util.List;

import com.execue.core.common.bean.Page;
import com.execue.core.common.type.AuditLogType;
import com.execue.core.common.type.CheckType;
import com.execue.core.common.type.OperatorType;

public class UserAccessAuditInput {

   private List<Long>         userIds;

   private List<AuditLogType> auditLogTypes;

   private List<Date>         searchDates;

   private CheckType          anonymousUser;

   private OperatorType       operator;

   private Page               page;

   public List<Long> getUserIds () {
      return userIds;
   }

   public void setUserIds (List<Long> userIds) {
      this.userIds = userIds;
   }

   public List<AuditLogType> getAuditLogTypes () {
      return auditLogTypes;
   }

   public void setAuditLogTypes (List<AuditLogType> auditLogTypes) {
      this.auditLogTypes = auditLogTypes;
   }

   public CheckType getAnonymousUser () {
      return anonymousUser;
   }

   public void setAnonymousUser (CheckType anonymousUser) {
      this.anonymousUser = anonymousUser;
   }

   public OperatorType getOperator () {
      return operator;
   }

   public void setOperator (OperatorType operator) {
      this.operator = operator;
   }

   public Page getPage () {
      return page;
   }

   public void setPage (Page page) {
      this.page = page;
   }

   /**
    * @return the searchDates
    */
   public List<Date> getSearchDates () {
      return searchDates;
   }

   /**
    * @param searchDates the searchDates to set
    */
   public void setSearchDates (List<Date> searchDates) {
      this.searchDates = searchDates;
   }

}
